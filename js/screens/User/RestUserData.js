...
import restUserDataModel from "models/restUserData.json";
...
export default class RestUserData extends React.Component {
  constructor(props) {
    super(props);
  }
  ...
  render() {
    ...
    const restUserDataFields = restUserDataModel.fields;
    const formData = {
      name: restUserDataModel.name,
      data: frmRestUserData,
    };
    return (
        ...
        >
          <Form form={formData}>
            ...
            <InputField
              ...
              //   { id: "2425", value: "sfsdfsdf" },
              // ]}
              schema={restUserDataFields.medicine_name}
              placeholder={"Date Of Birth"}
              onFocus={this.onFocusMedicine}
              onChange={this.handleFieldChange}
              error={restUserDataModelError}
            />
            <InputField
              data={[
                { id: "male", value: "Male" },
              ...
              schema={restUserDataFields.gender}
              placeholder={"Gender"}
              onChange={this.handleFieldChange}
              error={restUserDataModelError}
            />
            <InputField
              schema={restUserDataFields.height}
              placeholder={"Height"}
              onChange={this.handleFieldChange}
              error={restUserDataModelError}
            />
            <InputField
              schema={restUserDataFields.weight}
              placeholder={"Weight"}
              onChange={this.handleFieldChange}
              error={restUserDataModelError}
            />
            <InputField
              schema={restUserDataFields.blood_group}
  ...
  userDataHandler = () => {
    const { frmRestUserData, setState, saveMedicine, componentId } = this.props;

    const validateStatus = validate(restUserDataModel, { ...frmRestUserData });
    setState({
      restUserDataModelError: validateStatus,
    });

    if (isEmpty(validateStatus)) {
      setState({ btnState: true });
      saveRestUserData(frmRestUserData, componentId);
    }
  };
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    frmRestUserData: state.form.restUserData,
    sessionObject: state.app.sessionObject,
    isVisibleTimeModal: state.app.isVisibleTimeModal,
    restUserDataModelError: state.app.restUserDataModelError,
  };
};

export const RestUserDataContainer = connect(mapStateToProps, {
  setState,
  setFormFields,
  onFieldChange,
  resetForm,
